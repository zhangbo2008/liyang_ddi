...
class AE1(torch.nn.Module):  # Joining together
    ...
    def forward(self, X):
        X = self.dr((self.ac(self.l1(X))))

        X = self.att2(X)
        X = self.l2(X)

        X_AE = self.dr((self.ac(self.l3(X))))
...
class AE2(torch.nn.Module):  # twin network
    ...
    def forward(self, X):
        ...
        X1 = self.dr((self.ac(self.l1(X1))))
        X1 = self.att2(X1)
        X1 = self.l2(X1)
        X_AE1 = self.dr((self.ac(self.l3(X1))))
        X_AE1 = self.l4(X_AE1)

        X2 = self.dr((self.ac(self.l1(X2))))
        X2 = self.att2(X2)
        X2 = self.l2(X2)
        X_AE2 = self.dr((self.ac(self.l3(X2))))
        X_AE2 = self.l4(X_AE2)
