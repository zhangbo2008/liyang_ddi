...
#===========================当前行得到的df_feature就是onne-hot编码
    ...
    return       np.hstack([df_feature,sim_matrix])
...
class my_loss1(nn.Module):
    ...
    def forward(self, X, target, inputs, X_AE1, X_AE2, X_AE4):
        ...
        loss = calssific_loss_weight * self.criteria1(X, target) + \
               0*self.criteria2(inputs.float(), X_AE1) + \
               0*self.criteria2(inputs.float(), X_AE2) + \
               0*self.criteria2(inputs.float(), X_AE4)
        return loss
